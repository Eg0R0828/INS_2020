...
def generating(model=None, epoch='best'):
    ...
    if not model:
        ...
        model.add(tensorflow.keras.layers.LSTM(256, input_shape=(X.shape[1], X.shape[2])))
...
def preparation():
    ...
    checkpoint = tensorflow.keras.callbacks.ModelCheckpoint(filepath, monitor='loss', verbose=1, save_best_only=True, mode='min')
